...
function FlipText({ text, distance }: { text: string; distance: number }) {
  return (
    <span className="stat-flip">
      {text.split("").map((ch, i) => (
        <span
          key={`${distance.toFixed(1)}-${i}`}
          className="stat-flip__char text-[#01e044]"
        >
          {ch}
        </span>
      ))}
...
export default function ImpactStats({ distanceKm }: { distanceKm?: number }) {
  ...
  return (
    <>
      ...
      <div
        ref={placeholderRef}
        style={{ visibility: pinned ? "hidden" : "visible" }}
      >
        {renderStats()}
      </div>
      ...
      {pinned && (
        <>
          ...
          >
            <span className="stat-tab__icon">
              <Route className="h-3.5 w-3.5" strokeWidth={2.6} />
            </span>
            <span className="stat-tab__icon">
              <Leaf className="h-3.5 w-3.5" strokeWidth={2.6} />
            </span>
          </button>

          <div
            className={`stat-pinned ${collapsed ? "stat-pinned--collapsed" : ""}`}
          >
            {renderStats()}
          </div>
        </>
      ...
      <style jsx>{`
        ...
        .stat-tab:hover :global(.stat-tab__icon) {
